(function() {
    ...
    angular.module("productManagementApp")
        .controller("PriceAnalyticsController", ["$scope",
            "$filter",
            "products",
            "productService",
    ...
    function PriceAnalyticsController($scope, $filter, products, productService) {
        ...
        for (var i = 0; i < products.length; i++) {
            products[i].marginPercent = productService.calculateMarginPercent(products[i].price,
                products[i].cost);
            products[i].marginAmount = productService.calculateMarginAmount(products[i].price,
                products[i].cost);
        }
